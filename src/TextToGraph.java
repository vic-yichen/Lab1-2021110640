...
public class TextToGraph { // 定义名为 TextToGraph 的类
   ...
   * 展示生成的有向图.
   *
   * @param dotFilePath dotfilepath
   *
   * @param outputImagePath outputpath
   */
  public void showDirectedGraph(String dotFilePath, String outputImagePath) {
    try {
      ...
      BufferedReader errorReader
          = new BufferedReader(new InputStreamReader(process.getErrorStream())); // 读取进程的错误输出流
  ...
  public String queryBridgeWords(String word1, String word2) {
    ...
    if (bridgeWords.isEmpty()) { // 如果没有找到桥接词
      ...
      StringBuilder result = new StringBuilder("The bridge words from \""
          + word1 + "\" to \"" + word2 + "\" are: "); // 创建结果字符串
      int i = 0; // 计数器
      for (String word : bridgeWords) { // 遍历桥接词集合
        if (i > 0) {
          result.append(", "); // 如果不是第一个桥接词，则添加逗号和空格
        }
   ...
   * picsave.
   *
   * @param outputFile png
   *
   * @param shortestPaths paths
   *
   * @param pathLength weight
   */
  public void saveToDotFile_color(String outputFile,
                                    List<List<String>> shortestPaths, int pathLength) {
    List<String> dotLines = new ArrayList<>(); // 创建一个存储DOT文件行的列表
    List<String> color = new ArrayList<>(Arrays.asList("green", "orange", "pink", "yellow"));
    int numshortPath = shortestPaths.size(); // 获取最短路径的数量
    if (rootWord != null) { // 如果根单词不为空
    ...
    for (String from : graph.keySet()) { // 遍历图中的每个单词
      for (String to : graph.get(from).keySet()) { // 遍历当前单词的每个邻居
        ...
        int index1;
        int index2; // 创建变量以存储当前边的起始单词和目标单词在最短路径中的索引
        for (int i = 0; i < numshortPath; i++) { // 遍历每条最短路径
          List<String> shortestPath = shortestPaths.get(i); // 获取当前最短路径
          if ((index1 = shortestPath.indexOf(from)) != -1
              && (index2 = shortestPath.indexOf(to)) != -1) { // 如果当前边的起始单词和目标单词在最短路径中
        ...
        if (flag == -2) {
          dotLines.add(String.format("    \"%s\" -> \"%s\" [label=\"%d\", "
              + "color=\"yellow\"];", from, to, weight)); // 将当前边添加到DOT文件行中，并设置特殊颜色
        } else if (flag == -1) { // 如果当前边不在任何最短路径中
          dotLines.add(String.format("    \"%s\" -> \"%s\" [label=\"%d\"];",
              from, to, weight)); // 将当前边添加到DOT文件行中
          ...
          dotLines.add(String.format("    \"%s\" -> \"%s\" [label=\"%d\", color=\"%s\"];",
              from, to, weight, colorValue)); // 将当前边添加到DOT文件行中，并设置颜色
        }
      }
    }

    dotLines.add(String.format("    \"Path length = %d\" [label=\"Path length = %d\", "
        + "color=\"black\", shape=none];", pathLength, pathLength)); // 添加标记路径长度的行
  ...
  public List<List<String>> shortestPaths(String word1, String word2, int[] pathLength) {
    ...
    PriorityQueue<Map.Entry<String, Integer>> priorityQueue
        = new PriorityQueue<>(Map.Entry.comparingByValue()); // 创建一个优先级队列用于按距离排序
    ...
    findPaths(predecessors, word2, word1, path, resultPaths,
        resultWeights, distances.get(word2)); // 查找所有最短路径
  ...
  //p 是所有最短路径中节点的总数
  private void findPaths(Map<String, List<String>> predecessors, String current,
                           String start, LinkedList<String> path, List<List<String>> resultPaths,
                           List<Integer> resultWeights, int currentWeight) {
    ...
    if (current.equals(start)) { // 如果当前节点等于起始节点
      ...
      for (String predecessor : predecessors.get(current)) { // 遍历当前节点的前驱节点列表
        ...
        findPaths(predecessors, predecessor, start, path,
            resultPaths, resultWeights, currentWeight - edgeWeight); // 递归调用 findPaths 方法，探索前驱节点的路径
      }
    }
  ...
  public void calcShortestPath(String word1, String word2) {
    ...
    if (word1.isEmpty()) {
      System.out.print("No word1 \n");
    }
    if (word2.isEmpty()) {
      Map<String, Map<String, Integer>> graphData = getGraph(); // 获取图的数据
      for (String node : graphData.keySet()) { // 遍历图中的每个节点
        if (!node.equals(word1)) { // 如果节点不是第一个单词
          List<List<String>> paths = shortestPaths(word1, node, pathLength); // 计算到该节点的最短路径
          if (!paths.isEmpty()) { // 如果最短路径不为空
            for (List<String> path : paths) { // 遍历最短路径列表
              System.out.println("Shortest path: "
                  + String.join("->", path) + ", Weight: " + pathLength[0]); // 打印最短路径及其权重
            }
            String outputFile = String.format("./out/text/shortest_path_%s_to_%s.dot",
                word1, node); // 定义输出文件路径
            saveToDotFile_color(outputFile, paths, pathLength[0]); // 将带有标记路径的DOT文件保存到指定路径
            showDirectedGraph(outputFile, String.format("./out/png/shortest_paths_%s_to_%s.png",
                word1, node)); // 展示最短路径的图形
          } else { // 如果最短路径为空
            System.out.println("No shortest path found from "
                + word1 + " to " + node); // 打印未找到最短路径的消息
          }
        }
      }
    } else {
      List<List<String>> shortestPaths = shortestPaths(word1, word2, pathLength); // 计算两个单词之间的最短路径
      if (!shortestPaths.isEmpty()) { // 如果最短路径不为空
        for (List<String> path : shortestPaths) { // 遍历最短路径列表
          System.out.println("Shortest path: " + String.join("->", path)
              + ", Weight: " + pathLength[0]); // 打印最短路径及其权重
        }
        String outputFile = "./out/text/shortest_path.dot"; // 定义输出文件路径
        saveToDotFile_color(outputFile, shortestPaths, pathLength[0]); // 将带有标记路径的DOT文件保存到指定路径
        showDirectedGraph(outputFile, "./out/png/shortest_paths.png"); // 展示最短路径的图形
      } else { // 如果最短路径为空
